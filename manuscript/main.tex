...
Model selection criteria trade off a candidate model's fit to the observed data against its complexity. Archetypal examples are the Akaike Information Criterion (AIC; \citealt{akaike_new_1974}) and the Bayesian Information Criterion (BIC; \citealt{schwarz_estimating_1978}), which are among the earliest proposed criteria and remain widely used. Detailed expositions on model selection can be found in \cite{mcquarrie_regression_1998}, \cite{vapnik_statistical_1998}, \cite{vapnik_nature_2000}, \cite{claeskens_model_2001}, and \cite{buhlmann_statistics_2011}.
...
In this work, we seek to specialise and refine the PanIC framework towards consistent estimation methods for general regression problems. Consistency conditions for parameter estimators and minimal risks of regression problems under the empirical risk minimisation (ERM) approach are well established and can be found, for example, in \cite{vapnik_statistical_1998}, \cite{vapnik_nature_2000}, and \cite{shapiro_lectures_2021}. However, selecting model complexity solely according to the ERM principle tends to produce models that overfit the data. This fact has been widely recognised in the literature and has motivated the broad study of regularisation techniques. Examples of such methods include $l_2$ regularisation, leading to the Ridge regression problem \citep{hoerl_ridge_1970}; $l_1$ regularisation, leading to the Lasso problem \citep{tibshirani_regression_1996}; and a convex combination of $l_2$ and $l_1$ penalties, leading to the elastic-net problem \citep{zou_regularization_2005}. Results regarding the consistency of estimators obtained from solving such regularisation problems, and others, can be found, for example, in \cite{zhao_model_2006}, \cite{yuan_non-negative_2007}, \cite{jia_model_2010}, and \cite{hastie_statistical_2015}.
...
\begin{enumerate}
    ...
    \item We give a finite-sample implementation and a support-free, repeated-split cross-signed radius rule for the PanIC multiplier. A pointwise fixed-dimensional result establishes the root-sample-size rate of the cross-signed $L_1$ target, while uniformity over a fixed positive compact multiplier range ensures that calibration does not alter the first-order consistency target. A numerical study evaluates the rule in independent and correlated Gaussian and logistic designs and in a Poisson design, separately reporting constraint-radius, support-recovery, predictive, calibration, grid, solver, and runtime diagnostics.
...
The rest of this article proceeds as follows: in Section \ref{Sect: Method}, the PanIC and BIC-like IC framework is reviewed and our proposed estimation method is given with theoretical justification. In Section \ref{Sect: Asymptotic-distribution}, we derive second-order asymptotic distribution results for constrained estimators and discuss the non-Gaussian limits that can arise under binding constraints. In Section \ref{Sect: Simulation}, we evaluate the implementable calibration protocol in independent and correlated Gaussian and logistic designs and in a Poisson design, and give a numerical boundary and subsampling illustration. Finally, concluding remarks and an outline of future research directions are given in Section \ref{Sect: Conclusion}. Proofs, supporting technical arguments, and reproducibility details are collected in the Appendix.

\section{Method} \label{Sect: Method}
We review the PanIC and BIC-like IC frameworks in Section \ref{Sect: PanIC}. The connection between regularised regression estimation and ICs is established in Section \ref{Sect: Regression}. A formal justification of our proposed estimation method is presented in Section \ref{Sect: Theory}. Lastly, a computational method is given in Section \ref{Sect: Computations}.
...
as the empirical risk and the expected risk, respectively. This formulation includes the common special case in which $\ell_k(X;\beta_k)=\ell(h_k(X;\beta_k))$ for some scalar loss $\ell$. Examples of classic model selection problems under this formulation are presented by \cite{nguyen_panic:_2023}.
...
Observe that this result permits data-dependent calibration of $\kappa$; Appendix \ref{App: kappa calibration} gives a concrete cross-fitting procedure for such computation.
\end{bluechange}


\subsection{Regression Problems} \label{Sect: Regression}
This section connects constrained regression estimation with model selection in the PanIC framework. We observe pairs $(x,y)$, where the response satisfies $y\in \mathcal{Y}\subset \mathbb{R}$ and the covariate vector satisfies $x\in\mathcal{X}\subset \mathbb{R}^d$, for some positive integer $d$. Thus, in the PanIC notation, a single observation is the pair $Z=(X,Y)\in\mathcal{X}\times\mathcal{Y}$. The goal is to infer the underlying relationship from an observed sample $(x_i, y_i)_{i\in\left[n\right]}$. In the generalised linear model (GLM) framework, this relationship is encoded through a mean function $h(\cdot; \beta_0, \beta): \mathcal{X} \to \mathcal{M}$, where $\mathcal{M}$ denotes the mean-parameter space, parametrised by a slope vector $\beta\in\mathbb{R}^d$ and an intercept $\beta_0\in\mathbb{R}$. We work with a loss function $\ell(z;\beta_0,\beta)$, where $z=(x,y)$. Our focus is on linear, logistic, Poisson, and Gamma regressions. Collectively, these models cover common response types, including continuous, binary, count, and positive continuous data. Table \ref{Table: Regression} summarises the corresponding mean and loss functions.
\begin{table}[H]
...
\]
To mitigate the tendency of ERM to overfit, the Lasso regression, the Ridge regression, or the Elastic-net regression are often solved instead. We note that both Lasso and Ridge regression problems can be represented in a unified manner with the following formulation. For some $C\in\mathbb{R}_{\geq0}$, define
...
\end{split}
\end{align}
where $\mathbb{U}$ is a sufficiently large compact \blue{interval} in $\mathbb{R}$ that contains the optimal intercept $\beta^*_0$ in its interior. Setting $p=1$ in (\ref{Eqs: Lasso Ridge}) corresponds to the Lasso problem \citep{tibshirani_regression_1996}, and setting $p=2$ corresponds to the Ridge regression problem \citep{hoerl_ridge_1970}.
...
\end{split}
\end{align}
for some $\alpha \in (0, 1)$ and $C\in\mathbb{R}_{\geq0}$. Notably, both (\ref{Eqs: Lasso Ridge}) and (\ref{Eqs: Elastic Net}) are equivalent to a single hypothesis within the PanIC framework, dependent on a hyperparameter $C$. If we consider a strictly increasing sequence of nonnegative real numbers denoted by $(C_k)_{k\in\left[m\right]}$, we obtain a sequence of hypotheses. In particular, $(C_k)_{k\in\left[m\right]}$ strictly increasing ensures that the corresponding parameter spaces are nested, in the sense that $\mathbb{T}_k \subset \mathbb{T}_l$ whenever $k<l$.
...
We outline sufficient conditions for consistent model selection in linear, logistic, Poisson, and Gamma regression problems. These conditions verify A1--A3; the consistency conclusion then follows for any choice of penalty satisfying B1 and B2. Complete derivations are given in the Appendix.
...
\begin{itemize}
    ...
    \item[ii)] The same verification is immediate when both the covariate vector and the response have compact support compatible with the relevant response space. For Gamma regression, the compact response support must be bounded away from zero so that $\log Y$ is bounded.
\end{itemize}
\end{remark}
...
\]
and define $A_k(x)=B_{\mathbb{U}}+B_k\|x\|_2$. The following statement makes explicit the role of constraining the intercept to the compact interval $\mathbb U$.
...
The consistency property only requires a nonnegative bounded or slowly growing score $D_{k,n}$; a more specific degrees-of-freedom interpretation is model-specific. For Lasso least squares, the number of non-zero slope parameters is an unbiased degrees-of-freedom estimate for a fixed penalty multiplier under the Gaussian-model and design conditions of \citet{zou_degrees_2007}. At a data-dependent multiplier or a prescribed constraint radius, it is used here only as a generic complexity score. For Ridge least squares with an unpenalised intercept, let $\bar X$ denote the column-centred design matrix and let $\bar y$ be the centred response vector, with $(\bar y)_i=y_i-n^{-1}\sum_{j=1}^n y_j$. For a fixed $\lambda>0$, the penalised objective $n^{-1}\|\bar y-\bar X\beta\|_2^2+\lambda\|\beta\|_2^2$ has slope smoother complexity
\begin{equation}\label{Eqs: Ridge df}
D^{\mathrm{ridge}}(\lambda)
=\operatorname{tr}\{\bar X(\bar X^\top \bar X+n\lambda I)^{-1}\bar X^\top\}.
\end{equation}
At $\lambda=0$, the inverse is replaced by the Moore--Penrose pseudoinverse, giving $\operatorname{rank}(\bar X)$. Evaluating this trace at a data-dependent multiplier supplies a generic score, not automatically the degrees of freedom of the complete selection procedure.
...
Because coefficient-norm thresholds depend on predictor units, all continuous predictors should be centred and scaled, with the intercept left unpenalised. For Gaussian regression, the response or loss should likewise be put on a documented dimensionless scale. A convenient dimensionless threshold transform on a deterministic interval $[a,b]$ is $\psi(C)=(C-a)/(b-a)$; on a fixed grid with $m\geq2$, one may instead use the rank $(k-1)/(m-1)$. Any such strictly increasing transform preserves the consistency argument.
\end{bluechange}
...
So far, we have considered finitely many candidate models. We next allow hypotheses indexed by a compact interval, which requires a result for continuous, compact-valued correspondences. A correspondence $\varphi$ from a set $\mathbb{X}$ to a set $\mathbb{Y}$ assigns to each $x\in\mathbb{X}$ a subset of $\mathbb{Y}$ \citep[Definition 17.1]{aliprantis_infinite_2006}.
...
\]
for some positive integer $q$, where $a<b$. Let $\mathcal{L}:k\mapsto\mathbb{T}_{k}$ be a continuous correspondence with \blue{nonempty} compact values, and assume that the parameter spaces $(\mathbb{T}_{k})_{k\in\left[a,b\right]}$ are nested. Let $\ell:\mathbb{X}\times\mathbb{R}^{q}\to\mathbb{R}$ be a loss function and define
\[
...
\begin{equation}\label{Eqs: Parameter Space}
    ...
    \end{cases}
\end{equation}
With this definition, $\mathcal{L}: k \mapsto \mathbb{T}_k$ is compact-valued and continuous. Therefore, we have the following result.
...
In the linear-regression setting, suppose the conditions of Proposition \ref{Prop: BIC-like consistency} hold. Then the conclusion of Theorem \ref{Thm: Infinite selection} applies to any continuous strictly increasing BIC-like penalty on $[a,b]$ that satisfies B1 and $\mathrm{B2}^*$. In particular, for any fixed continuous strictly increasing $q:[a,b]\to\mathbb R_{>0}$, the penalty $P_{k,n}=q(k)\log(n)/n$ for $n\ge2$, together with any positive convention at $n=1$, satisfies these conditions.
\end{corollary}
...
Let $(\mathbb T_c)_{c\in[a,b]}$ be nested, and let $\mathcal G_n\subset[a,b]$ be a nonempty finite random set, possibly constructed from the same data used in $R_n$. Suppose there are measurable $c_n^+\in\mathcal G_n$ and nonnegative $\delta_n=o_{\mathbb P}(1)$ such that
...
In this section, we give a computational method for implementing PanIC and, for regularised linear regression, the BIC-like criterion in (\ref{Eqs: Modified BIC}) on a finite ordered candidate set satisfying the finite-grid conditions stated above.
...
$\beta\mapsto f\left(\beta\right)+\lambda g\left(\beta\right)$ admits a unique minimiser on $\mathbb{R}^{d}$. Then, the mapping $\mathcal{M}:\mathbb{R}_{\geq0}\to\mathbb{R}_{\geq0}$,
given by
\[
\mathcal{M}(\lambda)=g(\beta_{\lambda}),
\]
...
Our formulation of regression problems in the PanIC framework is associated
with a sequence of constrained optimisation problems, which can be
...
\]
Thus the fitted radius tends to zero as $\lambda\to\infty$, so a bracket exists for every $0<C<g(\beta_{\lambda=0})$ under the lemma's assumptions. With an intercept, the same argument uses a fixed feasible intercept at zero slope as the reference point. An inactive constraint returns the unpenalised fit, and $C=0$ returns the zero-slope fit. Numerical specifications and reproducibility materials are referenced in Appendix \ref{App: reproducibility}.
\end{bluechange}

\begin{bluechange}
The numerical study uses a warm-started \texttt{glmnet} path, augmented by the intercept-only and ordinary unregularised fits, and interpolates neighbouring solutions onto the radius grid. Interpolation approximates the exact constrained minimisers used in the theory. A small radius residual checks feasibility, not optimality. Implementation settings and direct-fit checks are provided with the code referenced in Appendix \ref{App: reproducibility}.

The numerical intercept is unpenalised and unconstrained. Agreement with the compact-intercept theory requires a fixed interval $\mathbb U$ containing the relevant population intercepts in its interior and consistent intercept estimates for the finite candidate collection. Under those conditions, the numerical estimates belong to $\mathbb U$ with probability tending to one. This localisation, and sufficiently accurate optimisation, are not established by a finite numerical check.

The empirical comparisons cover Gaussian, logistic, and Poisson regression. Gamma regression is treated analytically in Proposition \ref{Prop: estimated Gamma shape}, but is not included in the comparative simulations. A separate fixed-shape numerical check reported a convergence warning and is recorded with the code.
\end{bluechange}

The optimisation steps in Algorithm \ref{Algo: PanIC Lasso}
are the same whether the final selector is PanIC or the modified BIC.
In particular, once $\widehat v_{k,n}$ and $(\hat{\beta}_{0,k,n},\hat{\beta}_{k,n})$
are computed, one may evaluate either a PanIC penalty $P_{k,n}$,
or, in the linear regression setting, a modified BIC penalty term
\[
P_{k,n}=\frac{\log n}{n}\{c_0+\kappa_D\widetilde D_{k,n}+\varepsilon\psi(C_k)\},
\]
with $\widetilde D_{k,n}$ defined in (\ref{Eqs: cumulative complexity}),
and then minimise $\widehat v_{k,n}+P_{k,n}$ over $k$.
...
via the corresponding Lagrangian problem using the bracketed root-search protocol above; return the unpenalised fit when the constraint is inactive.
...
In this subsection, we provide the second-order asymptotic distribution
of constrained least squares estimators in linear regression. In contrast
to standard ordinary least squares (OLS) asymptotics, binding constraints
typically yield non-Gaussian limit distributions described by a random
quadratic optimisation problem over the critical cone.
...
\[
(\beta_{0,k}^{*},\beta_{k}^{*})=\argmin_{(\beta_{0},\beta)\in\mathbb{T}_{k}}r(\beta_{0},\beta),\qquad(\hat{\beta}_{0,k,n},\hat{\beta}_{k,n})=\argmin_{(\beta_{0},\beta)\in\mathbb{T}_{k}}R_{n}(\beta_{0},\beta),
\]
...
Write $\widehat\theta_{k,n}=(\widehat\beta_{0,k,n},\widehat\beta_{k,n}^\top)^\top$ for a measurable selection from the empirical constrained argmin. For a subset $J\subset[n]$ of size $b$, let $\widehat\theta_{k,b,J}$ be the corresponding measurable selection, using the same deterministic tie rule, computed from the observations indexed by $J$.

\begin{proposition}\label{Prop: subsampling fixed k}
Fix $k$ and suppose that Theorem \ref{Thm: Shapiro fixed k} applies to this fixed candidate. Let $a\in\mathbb R^{d+1}$ be fixed and let $F_{k,a}$ denote the nondegenerate distribution function of $a^\top\bar d_k(Z_k)$. Let the deterministic integers $1\le b=b_n\le n$ satisfy $b\to\infty$ and $b/n\to0$, let $\mathcal J_{n,b}=\{J\subset[n]:|J|=b\}$, and define
\[
\widehat L_{n,b,a}(x)
=\binom{n}{b}^{-1}\sum_{J\in\mathcal J_{n,b}}
\mathds 1\!\left\{\sqrt b\,a^\top(\widehat\theta_{k,b,J}-\widehat\theta_{k,n})\le x\right\}.
\]
Then, at every continuity point $x$ of $F_{k,a}$,
\[
\widehat L_{n,b,a}(x)\xlongrightarrow{\mathbb P}F_{k,a}(x).
\]
Let $B_n\to\infty$ be deterministic. Conditional on the data, draw $B_n$ independent uniform subsets from $\mathcal J_{n,b}$ with replacement, or a uniform sample of $B_n\le\binom nb$ distinct subsets without replacement. The empirical distribution based on these subsets has the same pointwise limit.
\end{proposition}

The result is an application of the i.i.d. subsampling theorem and its stochastic-approximation corollary in \citet[Theorem 2.2.1 and Corollary 2.4.1]{politis1999subsampling}, with normalising sequence $\tau_n=\sqrt n$. The condition $b/n\to0$ gives $\tau_b/\tau_n=\sqrt{b/n}\to0$. Applying the same subsampling theorem directly to the scalar estimator in Proposition \ref{Prop: scalar boundary} gives $a=1$. Its subsampling distribution converges at every $x\ne0$ to the distribution of $\min\{G,0\}$. Consequently, its quantiles that lie strictly below zero are consistently estimated at probability levels where the limiting distribution function is continuous and strictly increasing; no quantile-consistency conclusion is asserted at the jump at zero. For vector-valued uncertainty, the norm version of the same construction follows from \citet[Theorem 2.5.2]{politis1999subsampling} whenever the limiting norm distribution is continuous at the quantile of interest. A simple admissible schedule is $b_n=\lfloor n^\gamma\rfloor$ with $0<\gamma<1$; finite-sample sensitivity to $b_n$ should nevertheless be assessed.
\end{bluechange}
We now record the immediate consequence of Theorem \ref{Thm: Shapiro fixed k}
for selectors over a finite grid. The interval-indexed case requires
additional localisation control, which we discuss in the sequel.
...
\begin{bluechange}
The primary comparison is among four implementable selectors: PanIC with the repeated two-way cross-signed multiplier rule in Appendix \ref{App: kappa calibration}, denoted PanIC-CF; the MSE-scale BIC-like criterion for Gaussian regression, with an analogue based on counts of non-zero slope parameters reported only as an exploratory comparator for logistic and Poisson regression; and two selectors computed from the same ordinary five-fold cross-validation losses, denoted CV-min and CV-1SE \citep{stone_cross-validatory_1974,hastie_elements_2009}. Proposition \ref{Prop: uniform kappa} shows that restricting the data-dependent multiplier to a fixed positive compact range preserves the first-order PanIC target. It does not assert that radius matching is optimal for prediction or support recovery, so these aspects are evaluated separately below. Support-informed choices of $\kappa$ are retained only as explicitly labelled supplementary post hoc oracle diagnostics and are not used in any primary table.

\subsection{Simulation design}\label{Sect: Linear regression}
Every setting has dimension $d=20$, with non-zero slope parameters indexed by $A^*=\{1,3,\ldots,19\}$. The ten nonzero entries alternate in sign and are rescaled so that $\beta^{*\top}\Sigma\beta^*=1$; throughout the simulation study, $C^\star=\|\beta^*\|_1$ denotes the corresponding true constraint radius. For the independent design, $\Sigma=I_{20}$ and $C^\star=\sqrt{10}\approx3.1623$. For the correlated design, $\Sigma_{rs}=0.5^{|r-s|}$, whose diagonal entries are all one, and the rescaling gives $C^\star\approx3.9778$. Thus every simulated coordinate is already on the same population scale, and no empirical predictor standardisation is applied. Gaussian responses have intercept zero and noise standard deviation $\sigma=1$; their selection loss is divided by $\sigma^2$. Logistic responses have intercept zero. The Poisson design is independent Gaussian with
\[
\beta_0^*=\log(2)-\tfrac12\beta^{*\top}\Sigma\beta^*\approx0.1931,
\]
so that the marginal count mean is two. The independent Gaussian and logistic settings use $n\in\{500,2000\}$, the correlated Gaussian and logistic settings use $n=1000$, and the Poisson setting uses $n=1000$.

Each setting has $N=1{,}000$ Monte Carlo replications and an independent test sample of size 2,000 per replication. All selectors use the same fixed candidate grid $C_k=(k-1)/6$, $k=1,\ldots,121$, spanning $[0,20]$. This interval contains every simulated true radius. We use $\psi(C)=C/20$ and the PanIC shape $(1+\psi(C))\sqrt{\log n/n}$. The multiplier grid consists of 31 log-spaced values from $10^{-2}$ to $10^2$. PanIC-CF-original is a same-path sensitivity analysis that replaces the square-root log-log discrepancy weight in (\ref{Eqs: fold radius discrepancy}) by the log-log weight; it requires no additional model fit and does not enter the primary joint decision. The Gaussian BIC-like criterion uses $\kappa_D=1$, the cumulative non-zero parameter count, and $\varepsilon=10^{-6}$ in (\ref{Eqs: Modified BIC}). For compact table labelling, the logistic and Poisson analogues based on non-zero parameter counts are also denoted BIC-like; they use the likelihood-scale coefficient $1/2$ multiplying the same cumulative-count expression, are exploratory comparators, and are not covered by Proposition \ref{Prop: BIC-like consistency}.

Computational code and specifications are referenced in Appendix \ref{App: reproducibility}.
...
For the cross-validation comparators, let $\mathcal I_1,\ldots,\mathcal I_V$, with $V=5$, be the balanced folds generated from the recorded CV seed. For candidate radius $C_k$, let $\widehat\vartheta_{-v,k}$ denote the constrained fit obtained without fold $\mathcal I_v$, and define
\[
\widehat L_{v,k}
=\frac{1}{|\mathcal I_v|}
...
\qquad
\widehat{\operatorname{se}}_{\rm CV}(k)
=\left[
\frac{1}{V(V-1)}
\sum_{v=1}^V
\{\widehat L_{v,k}-\overline L_k\}^2
\right]^{1/2}.
\]
With $C_1<\cdots<C_m$, define
\[
\widehat k_{\rm CV\text{-}min}
=\min\left\{\argmin_{k\in[m]}\overline L_k\right\}
\]
and
\[
\widehat k_{\rm CV\text{-}1SE}
=\min\left\{
k\in[m]:
\overline L_k
\leq
\overline L_{\widehat k_{\rm CV\text{-}min}}
+\widehat{\operatorname{se}}_{\rm CV}
(\widehat k_{\rm CV\text{-}min})
\right\}.
\]
Thus CV-min chooses the smallest-radius minimiser in an exact tie, whereas CV-1SE chooses the smallest radius whose mean validation loss is within one descriptive across-fold standard error of the CV-min value. The five training samples overlap, so $\widehat{\operatorname{se}}_{\rm CV}$ is a selection heuristic rather than an independent-fold inferential standard error. CV-min and CV-1SE use identical folds, fitted paths, losses, candidate radii, preprocessing, and solver settings. Each selected radius is refitted on the full sample before evaluation on the same independent test sample; the independent test sample does not enter either selection rule.

For the selected index set of non-zero slope parameters $\widehat A=\{j:|\widehat\beta_j|>10^{-8}\}$, we report
\[
...
\]
The signed attained-radius error is $\|\widehat\beta\|_1-\|\beta^*\|_1$; under each correctly specified simulation, the data-generating slope $\beta^*$ is also the unregularised population minimiser $\beta^\circ$. The selected grid radius $C_{\rm sel}$ and attained fitted radius are reported separately because the constraint can be inactive. Predictive performance is evaluated only on the independent test sample. For Gaussian regression, test deviance is squared error divided by $\sigma^2$; for logistic and Poisson regression, it is the usual mean model deviance. No support or test-sample quantity enters the PanIC-CF calibration.

The production comparison was locked to a bounded joint claim against CV-min. For setting $s\in\{1,\ldots,7\}$ and paired replication $r$, let $W^{\rm P}_{s,r}$ and $W^{\rm C}_{s,r}$ be the total support errors $\mathrm{FP}+\mathrm{FN}$ for PanIC-CF and CV-min, and let $D^{\rm P}_{s,r}$ and $D^{\rm C}_{s,r}$ be their independent-test deviances. The two equal-weight seven-setting estimands are
\[
\Delta_{\rm S}
=\frac17\sum_{s=1}^7
\mathbb E\!\left(W^{\rm P}_{s,r}-W^{\rm C}_{s,r}\right),
\qquad
\Delta_{\rm D}
=\frac17\sum_{s=1}^7
\mathbb E\!\left(
\frac{D^{\rm P}_{s,r}-D^{\rm C}_{s,r}}{D^{\rm C}_{s,r}}
\right).
\]
The computational rule evaluates the relative contrast only when every comparator deviance is finite and strictly positive. A nonfinite or nonpositive value causes analysis and verification to fail before the joint decision is formed; no replication is discarded and no artificial denominator is substituted. All 7,000 production CV-min deviances passed this check, with minimum $0.91004$ and maximum $1.30181$. All realised relative contrasts were finite, their largest absolute value was $0.04451$, and their sample second moment was $7.62\times10^{-6}$. The normal-approximation interpretation additionally requires $D^{\rm C}_{s,r}>0$ almost surely and a finite second moment of the relative contrast under the stated simulation design; the finite production audit checks the realised calculation rather than purporting to prove those population conditions.

The support component declares superiority only when the normal-approximate one-sided $95\%$ upper Monte Carlo bound for $\Delta_{\rm S}$ is below zero. The prediction component declares noninferiority only when the corresponding upper bound for $\Delta_{\rm D}$ is below $0.001$, a $0.1\%$ relative-deviance margin. Scenario streams are disjoint, and the MCSE of each equal-weight estimate combines the seven scenario-specific paired MCSEs in quadrature and divides by seven. For either contrast, the normal-approximate upper bound is $\widehat\Delta+\Phi^{-1}(0.95)\widehat{\operatorname{MCSE}}(\widehat\Delta)$, where $\Phi$ is the standard normal distribution function. Its asymptotic Monte Carlo interpretation requires independent replications and a finite variance for the contrast. The joint conclusion requires both components to pass and is an intersection--union rule for the alternative $\Delta_{\rm S}<0$ and $\Delta_{\rm D}<0.001$.
...
independent-test summaries for the prespecified methods.
...
The reported summaries give different orderings for support and prediction. For the seven paired PanIC-CF versus CV-min comparisons, the mean total-support-error differences (PanIC-CF minus CV-min) and their Monte Carlo standard errors (MCSEs), in table order, were $-0.744$ ($0.047$), $-0.478$ ($0.057$), $-0.180$ ($0.043$), $-0.230$ ($0.041$), $-0.727$ ($0.049$), $-0.158$ ($0.039$), and $-0.550$ ($0.055$). These scenario summaries are secondary and are not simultaneous confidence statements. For the prespecified equal-weight primary estimand, $\widehat\Delta_{\rm S}=-0.43814$ with MCSE $0.01800$ and one-sided $95\%$ upper Monte Carlo bound $-0.40854$. Thus the support-superiority component against CV-min passes.

For the prediction guardrail, $\widehat\Delta_{\rm D}=0.000160$ with MCSE $0.000033$ and one-sided $95\%$ upper bound $0.000214$, which is below the prespecified $0.001$ margin. The point estimate and upper bound correspond to average relative test-deviance increases of approximately $0.0160\%$ and $0.0214\%$, respectively. Together with the support result, both components therefore pass the locked joint criterion for this simulation design, subject to the complete-pairing and denominator-validity rule stated above.

The two prespecified endpoint decisions are collected in
Table \ref{Table: confirmatory decision}.
% BEGIN PANIC INLINE TABLE: table_confirmatory_decision.tex
\begin{table}[H]
\centering
...
\end{adjustbox}
\end{table}
% END PANIC INLINE TABLE: table_confirmatory_decision.tex


The same-path PanIC-CF-original sensitivity had higher mean support error than PanIC-CF in every setting, by $0.206$ to $0.315$ errors per replication, while their mean test deviances were close. CV-1SE was much sparser and had lower mean support error than PanIC-CF in every setting, but its mean test deviance exceeded that of CV-min in every setting; it is therefore reported as a different support--prediction trade-off and is not the comparator in the primary joint claim. The rows labelled BIC-like were also substantially sparser, with mean total support errors from $2.18$ to $3.41$ and exact-recovery proportions from $0.067$ to $0.160$, at the cost of larger mean test deviance than CV-min. In the logistic $n=500$ setting, the non-Gaussian analogue based on non-zero parameter counts had a mean of $1.58$ false negatives. These reported comparisons do not give a universal ranking: matching the unregularised population radius, recovering a sparse support, and minimising predictive loss are different objectives. In particular, the first-order constraint-level consistency target is not a variable-selection guarantee.
\end{bluechange}

\begin{bluechange}
\subsection{Calibration and numerical diagnostics}
Table \ref{Table: revised calibration} reports the calibration targets,
selected multipliers, and failure diagnostics.
...
Across the 7,000 primary replications, no fitting or calibration failure or solver warning was reported, and the multiplier default was not used. One of the 70,000 raw cross-signed targets fell below zero and was projected onto the scoring interval. Neither PanIC-CF multiplier variant selected a multiplier or radius endpoint. The logistic BIC-like comparator selected $C=0$ in 95 of the 1,000 replications at $n=500$. Calibration diagnostics include the pilot radii and selected multipliers. The remaining numerical checks are supplied with the code.

Direct-fit validation and the separate fixed-shape Gamma check are documented with the computational materials referenced in Appendix \ref{App: reproducibility}.
...
The grid-sensitivity study uses 500 replications of the Gaussian independent setting at $n=1000$, with common random numbers and 61, 121, and 241 equally spaced radii on $[0,20]$ (Table \ref{Table: grid sensitivity}). PanIC-CF's mean total support error decreased from $6.204$ to $6.058$ to $5.878$, and its mean selected radius changed from $3.132$ to $3.122$ to $3.110$. The paired support-error changes were $-0.146$ (MCSE $0.079$) from 61 to 121 points and $-0.180$ ($0.056$) from 121 to 241 points. Mean test deviance was $1.020$, $1.019$, and $1.019$; the corresponding CV-min values were $1.019$ at the displayed precision. PanIC-CF-original had mean total support errors $6.454$, $6.338$, and $6.172$, while the BIC-like values were $2.678$, $2.410$, and $2.192$. Mean shared fitting time increased from $0.0674$ to $0.0888$ to $0.1310$ seconds. Thus the predictive comparison is stable over these three grids, whereas support error, selected radius, and computation retain some grid-resolution dependence; this finite comparison is not extrapolated to an asymptotic refining-grid claim.

\subsection{Boundary-law and subsampling illustration}
We illustrate Proposition \ref{Prop: scalar boundary} and the scalar subsampling construction following Proposition \ref{Prop: subsampling fixed k} with $\beta^\circ=C=1$ and independent $X,\epsilon\sim N(0,1)$. Thus $q=\tau^2=1$ and the limiting continuous component is standard Gaussian on the negative half-line. For each of $n=100$ and $n=500$, 40,000 independent samples were generated. Table \ref{Table: boundary illustration} and the first two panels of Figure \ref{fig:boundary subsampling} show that the empirical boundary mass and lower quantiles are close to their analytic values. These calculations are descriptive checks of the fixed-threshold limit, not a claim of uniform finite-sample approximation.
...
For the subsampling illustration, a single full sample of size $n=2000$ was generated once from the prespecified seed and used without selection or regeneration. Its unconstrained least-squares estimate was $1.0060$, so the constrained full-sample estimate under $|\beta|\le1$ was the binding value $\widehat\beta_n=1$. For each $b\in\{100,200,400\}$, each of $B_n=10{,}000$ independent repeated draws sampled $b$ distinct observation indices uniformly without replacement; the same subset could in principle recur across draws. Table \ref{Table: subsampling sensitivity} reports lower quantiles and CDF values at three negative continuity points. In this realised sample, $b/n=0.05$ was closest to the limiting CDF among the three choices. This finite comparison is consistent with, but is not implied monotonically by, the asymptotic condition $b/n\to0$. The reported atom at zero is descriptive only: Proposition \ref{Prop: subsampling fixed k} makes no pointwise approximation claim at that discontinuity.
...
\begin{figure}[H]
    ...
    \caption{Scalar binding-boundary and subsampling illustration. The first two panels compare the empirical negative component and boundary atom of $\sqrt n(\widehat\beta_n-\beta^\circ)$ with the analytic limit for $n=100$ and $n=500$. The third panel compares subsampling CDFs from the prespecified-seed full sample of size $n=2000$, using $B_n=10{,}000$ repeated random subsets for each $b\in\{100,200,400\}$, with the limiting CDF.}
...
\begin{figure}[H]
    ...
    \caption{Elapsed fitting time at $n=1000$ on the 121-point radius grid. Each boxplot contains 20 one-worker timed runs after two untimed warm-up runs. Data generation and independent-test evaluation are excluded. PanIC-CF includes ten half-sample paths and one full-sample path; PanIC-CF-original reuses those fits. The displayed five-fold-CV time is the shared computation for CV-min and CV-1SE: five fold-specific paths and one full-sample path. The Gaussian BIC-like criterion and the non-Gaussian analogues based on non-zero parameter counts each include one full-sample path and criterion evaluation. The computing environment is recorded with the code referenced in Appendix \ref{App: reproducibility}.}
    \label{fig:runtime}
\end{figure}

In this implementation, PanIC-CF had higher mean elapsed time than the shared five-fold-CV computation because it fits ten half-sample paths, ten training-sign GLMs, ten independent validation GLMs, and a full-sample path. The PanIC-CF and shared CV means (MCSEs), respectively, were $0.0542$ ($0.0008$) and $0.0299$ ($0.0006$) seconds for Gaussian regression, $0.1864$ ($0.0029$) and $0.1148$ ($0.0020$) seconds for logistic regression, and $0.1684$ ($0.0021$) and $0.0951$ ($0.0009$) seconds for Poisson regression. Selecting both CV-min and CV-1SE from the same fold-loss matrix requires no additional fit; likewise, PanIC-CF-original differs from PanIC-CF only in deterministic scoring of the same paths. The corresponding one-path means were $0.0052$ ($0.0002$) seconds for the Gaussian BIC-like criterion and $0.0209$ ($0.0002$) and $0.0172$ ($0.0002$) seconds for the logistic and Poisson analogues based on non-zero parameter counts. These timings compare the stated implementations on one machine, not general computational complexity.

The verified replication materials regenerate the seven manuscript-table mirrors and both figures. The manuscript embeds exact copies of the seven table mirrors, and the repository checks every marked block byte-for-byte against its generated counterpart; the separate \texttt{results/table\_*.tex} files are reproducibility outputs rather than compilation dependencies. Support-informed multiplier choices are retained only as labelled supplementary oracle outputs; because they use the true support, they are excluded from the primary comparisons and from the substantive conclusions.
\end{bluechange}
...
The first-order theory covers deterministic continuum-indexed nested families and finite random candidate sets satisfying explicit one-sided coverage, empirical-risk, and penalty-separation conditions. At the second order, the constrained least-squares estimator is described by a random quadratic programme over a critical cone; the scalar binding-boundary case has a closed form, and subsampling consistently approximates nondegenerate scalar projections for a fixed candidate. Exact finite-grid selection transfers the fixed-candidate law, whereas a continuum-selected second-order limit would require a separate localisation and stochastic-equicontinuity analysis.

The compact-multiplier result permits finite-sample calibration without changing the first-order target, and Appendix \ref{App: kappa calibration} supplies a repeated two-way cross-signed rule based on ordinary unregularised half-sample fits. The numerical study evaluates that implementable rule in independent and correlated Gaussian and logistic designs and in an independent Poisson design, without using the true support. Against CV-min, the prespecified equal-weight seven-setting estimand gave a mean PanIC-CF support-error difference of $-0.43814$ with a one-sided $95\%$ upper Monte Carlo bound of $-0.40854$. Its mean relative test-deviance difference was $0.000160$, with upper bound $0.000214$ below the $0.001$ noninferiority margin. Thus the bounded joint support-superiority and prediction-noninferiority criterion passed for this simulation design under the complete-pairing and denominator-validity gate. CV-1SE produced a different trade-off, with substantially lower support error but higher test deviance, and the BIC-like rows were sparser but also had larger mean test deviance than CV-min. These comparisons do not imply a general dominance ordering because constraint-radius estimation, support recovery, and prediction remain different targets. The binding-boundary and subsampling experiments illustrate the non-Gaussian limit and its approximation at continuity points. Further work includes a joint second-order limit for a continuum-selected threshold and its estimator, and high-dimensional calibration when ordinary unregularised half-sample fits are unavailable.
\end{bluechange}
...
\section{Repeated cross-signed calibration of the PanIC multiplier}\label{App: kappa calibration}
\begin{bluechange}
...
For the Lasso functional, the plug-in radius $\|\widetilde\beta_S\|_1$ has a folded contribution $|\widetilde\beta_{S,j}|$ from every null coordinate. The implemented calibration instead separates sign estimation from coefficient estimation. For disjoint nonempty samples $S$ and $T$, define $\operatorname{sign}(0)=0$ and
...
The two-way use of each split follows the general cross-fitting device of exchanging training and evaluation roles \citep{ChernozhukovEtAl2018DML}. Repeating the split reduces dependence on an arbitrary partition, as in multi-split procedures \citep{MeinshausenEtAl2009}. Related estimation theory for nonsmooth $L_1$ functionals appears in \citet{CaiLow2011}. These references motivate aspects of the construction only: they do not establish (\ref{Eqs: cross signed radius}) or import double-machine-learning orthogonality or multi-split $p$-value theory. Proposition \ref{Prop: cross signed pilot radius} proves the rate needed here. Because the $L_1$ functional is nondifferentiable at zero, no uniform local-to-zero claim is made \citep{HiranoPorter2012}.
...
\[
\mathbb P\left[
\min\left\{\argmin_{k\in[m]}D_N(k)\right\}=k^*
\right]\longrightarrow1,
\]
where $k^*$ is defined in Proposition \ref{Prop: population pilot radius}.
\end{proposition}


\begin{proof}
Fix $k<k^*$. Strict increase of $\psi$ and the definition of $k^*$ give $\delta_k=t^\circ-\psi(C_k)>0$. Since $t_N\to_{\mathbb P}t^\circ$, with probability tending to one, $t_N-\psi(C_k)\ge\delta_k/2$. Hence $D_N(k)\ge\omega_N\delta_k^2/4\to\infty$ in probability.

For $k\ge k^*$, first suppose $C_{k^*}>C^\circ$. With probability tending to one, $t_N<\psi(C_{k^*})$, and therefore
\[
D_N(k)=\{\psi(C_k)-t_N\}^2
\xlongrightarrow{\mathbb P}
\{\psi(C_k)-t^\circ\}^2.
\]
The limiting value is uniquely minimised over $k\ge k^*$ at $k=k^*$ because $\psi$ is strictly increasing. If instead $C_{k^*}=C^\circ$, then
\[
D_N(k^*)
\le \max(1,\omega_N)(t_N-t^\circ)^2
=O_{\mathbb P}(\omega_N/N)=o_{\mathbb P}(1),
\]
whereas for every $k>k^*$,
\[
...
Proposition \ref{Prop: asymmetric pilot target} concerns direct minimisation of the discrepancy over the candidate radii. In the multiplier rule below, a radius is reachable only if it is selected by PanIC for at least one $\kappa\in\mathcal K$; no assumption is made that every grid index is reachable. Accordingly, the proposition supplies the target-alignment rationale for the discrepancy, not a separate consistency theorem for the finite multiplier search. The first-order validity of the final full-sample selector follows instead from Proposition \ref{Prop: uniform kappa}.
...
Let $\mathcal A_n$ be the measurable event that every ordinary half-sample fit and every constrained row fit needed to compute the scores below returns finite values and passes the prespecified diagnostics; all ties and diagnostic decisions use fixed deterministic rules.

Let $\mathcal K=\{\kappa_1<\cdots<\kappa_L\}\subset[\underline\kappa,\overline\kappa]$ be fixed before inspecting the simulation outcomes, where $0<\underline\kappa\le\overline\kappa<\infty$. On $\mathcal A_n$, for each row compute ordinary unregularised fits on both halves,
\[
...
\qquad k\in[m].
\]
For each $\kappa\in\mathcal K$, define
\[
\widehat k_b(\kappa)
...
\[
\kappa_{\min}
=\min\left\{\argmin_{\kappa\in\mathcal K}\overline D(\kappa)\right\},
\]
and, on $\mathcal A_n$, select
\begin{equation}\label{Eqs: kappa one standard error}
\widehat\kappa_n^{\mathrm{cal}}
=\max\left\{\kappa\in\mathcal K:
\overline D(\kappa)
\le \overline D(\kappa_{\min})
+\widehat{\operatorname{se}}\{\overline D(\kappa_{\min})\}
\right\}.
\end{equation}
Equation (\ref{Eqs: kappa one standard error}) defines $\widehat\kappa_n^{\mathrm{cal}}$ on $\mathcal A_n$. Fix $\kappa^\dagger\in\mathcal K$ before observing the data (the implementation uses $\kappa^\dagger=1$) and define the total multiplier
\[
\widehat\kappa_n=
\begin{cases}
...
\end{cases}
\]
On $\mathcal A_n^c$ the procedure retains a calibration-failure flag; $\kappa^\dagger$ is a multiplier fallback and no regularised pilot is substituted. Thus $\widehat\kappa_n\in\mathcal K\subset[\underline\kappa,\overline\kappa]$ on every outcome, so Proposition \ref{Prop: uniform kappa} applies directly. The cross-signed pilot-rate interpretation additionally requires $\mathbb P(\mathcal A_n)\to1$ and the half-sample rate conditions in Proposition \ref{Prop: cross signed pilot radius}; that success-probability condition is not needed for the uniform-multiplier conclusion once the rule is totalised.
The particular admissible sequence used for $\omega_b$ can change which element of $\mathcal K$ is selected, but it cannot affect this compact-range argument.
...
No convergence of $\widehat\kappa_n$ to a unique population-optimal multiplier is required or claimed. The radius-matching score is a finite-sample calibration device; preservation of the first-order PanIC target follows only from the total multiplier's fixed positive compact range.

\begin{remark}\label{Rem: pilot existence}
Calibration from the pilot scores requires a unique finite ordinary unregularised fit on both halves of every repeated split. In logistic regression, complete or quasi-complete separation can preclude a finite maximum-likelihood estimate; see \citet{albert_anderson_1984}. Analogous nonexistence or nonuniqueness may occur in other GLMs. If any required fit fails, the procedure records a calibration failure and applies the prespecified $\kappa^\dagger$ convention solely to keep the selector defined.
\end{remark}
\end{bluechange}

\begin{bluechange}
\section{Reproducibility details}\label{App: reproducibility}
The accompanying replication archive is \texttt{PanIC\_R1\_replication\_archive.zip}; its SHA-256 digest is \texttt{REPLICATION\_ARCHIVE\_SHA256\_PENDING}. It contains the locked configuration and checksums, deterministic seed ledgers, source scripts, raw scenario files, paired contrasts, the relative-deviance denominator audit, calibration and failure diagnostics, solver checks, environment and session records, and the generated table mirrors and figures. The seven table bodies are embedded directly in \texttt{main.tex}; their separate repository copies are checksum-controlled reproducibility mirrors, not compilation dependencies.

A compact public repository containing the numerical implementation, manuscript source, retained outputs, deterministic table renderer, and verification checks is available at \url{https://github.com/hiendn/PanIC_for_Regularised_GLMs}; commit \texttt{IMPLEMENTATION\_COMMIT\_PENDING} identifies the implementation corresponding to the reported outputs.
\end{bluechange}
...
\begin{proof}
For $n\ge2$, positivity is immediate; at $n=1$ it follows from the positive convention in the proposition. Since $m$ is fixed and $\psi(C_m)<\infty$,
\[
...
\]
whose distribution function is $F_{k,a}$. Apply \citet[Theorem 2.2.1]{politis1999subsampling} with normalising constants $\tau_n=\sqrt n$. The conditions $b\to\infty$ and $b/n\to0$ imply $\tau_b/\tau_n=\sqrt{b/n}\to0$, and the subsample statistic in that theorem is exactly $\sqrt b\,a^\top(\widehat\theta_{k,b,J}-\widehat\theta_{k,n})$. The theorem therefore yields $\widehat L_{n,b,a}(x)\to_{\mathbb P}F_{k,a}(x)$ at every continuity point of $F_{k,a}$. If only $B_n$ subsets are sampled, the stochastic-approximation result of \citet[Corollary 2.4.1]{politis1999subsampling} applies when $B_n\to\infty$ and gives the same limit.
\end{proof}
\end{bluechange}
...
of $\hat{K}_{n}$ is non-empty and $\hat{K}_{n}$ is well-defined. For each fixed $k$, the measurable maximum theorem gives measurability of the criterion in the sample, while its sample paths are continuous in $k$; hence the criterion is jointly measurable by the Carath\'eodory measurability theorem. The argmin correspondence on the compact interval therefore admits a measurable selector, and the minimum tie-breaking selector $\hat K_n$ is measurable; see \citet[Theorem 18.19]{aliprantis_infinite_2006}.
...
\begin{proof}
    ...
    \begin{enumerate}
    \item[A1]: $\ell_k(x, y; \cdot): \mathbb{T}_k \to \mathbb{R}$ is continuous for each $(x, y) \in \mathbb{X}$ as it is differentiable with respect to $\beta$. Fix $\beta \in \mathbb{T}_k$, $\ell_k(\cdot; \beta): \mathbb{X} \to \mathbb{R}$ is given by
    \begin{align*}
        \ell_k(x, y; \beta) &= (\Tilde{f}(x, y; \beta)  - \Tilde{g}(x, y; \beta))^2\\
        &= (f(y; \beta)  - g(x; \beta))^2,
    \end{align*}
    where $\Tilde{f}(x, y; \beta) = f(y; \beta) = y$ and $\Tilde{g}(x, y; \beta) = g(x; \beta) = \beta^\top x$. The functions $f$ and $g$ are measurable on their respective domain, so $\Tilde{f}$ and $\Tilde{g}$ are measurable on the product space $\mathbb{X}$. It follows that $\ell_k(\cdot; \beta)$ is measurable on $\mathbb{X}$.
    \item[A2]: $\mathbb{T}_k = \{\beta\in \mathbb{R}^d: \lVert\beta\rVert_1 \leq C_k\}$ is compact. Let $\tau_k = 0 \in \mathbb{R}^d$, as $\lVert 0 \rVert_1 \leq C_k$ for all $k \in \left[m\right]$, $\tau_k \in \mathbb{T}_k$. This condition is satisfied if $Y$ has a finite fourth moment:
    \[\mathbb{E}\left(\ell_k(X, Y; \tau_k)^2\right) = \mathbb{E}\left(Y^4\right) < \infty.\]
    \item[A3]: Fix $(x, y)\in \mathbb{X}$, $k \in \left[m\right]$ and $\beta, \beta' \in \mathbb{T}_k$. By the multivariate Mean Value Theorem \citep{davidson_stochastic_2021},
...
\begin{proof}
    ...
    \item[A1]: $\ell_k(x, y; \cdot): \mathbb{T}_k \to \mathbb{R}$ is continuous for each $(x, y) \in \mathbb{X}$ as it is differentiable with respect to $\beta$. Fix $\beta \in \mathbb{T}_k$, $\ell_k(\cdot; \beta): \mathbb{X} \to \mathbb{R}$ is given by
    \begin{align*}
        \ell_k(x, y; \beta) &= \Tilde{f}(x, y; \beta)\Tilde{g}(x, y; \beta)  + \Tilde{h}(x, y; \beta)\\
        &= f(y; \beta)g(x; \beta)  + h(x; \beta),
    \end{align*}
    where $\Tilde{f}(x, y; \beta) = f(y; \beta) = y$,
    \begin{align*}
        \Tilde{g}(x, y; \beta) = g(x; \beta) &= -\log\left(\frac{1}{1 + e^{-\beta^\top x}}\right) + \log\left(1 - \frac{1}{1 + e^{-\beta^\top x}}\right),\\
        \Tilde{h}(x, y; \beta) = h(x; \beta) &= -\log\left(1 - \frac{1}{1 + e^{-\beta^\top x}}\right).
    \end{align*}
    Both $g$ and $h$ are continuous functions, so they are measurable with respect to the Borel $\sigma$-algebra on $\mathbb{R}^d$. The function $f$ is measurable with respect to the discrete $\sigma$-algebra on $\{0, 1\}$. So $\Tilde{f}$, $\Tilde{g}$ and $\Tilde{h}$ are measurable on the product space $\mathbb{X}$. It follows that $\ell_k(\cdot; \beta)$ is measurable on $\mathbb{X}$.
    \item[A2]: $\mathbb{T}_k$ is compact. Let $\tau_k = 0 \in \mathbb{R}^d$, then $\tau_k \in \mathbb{T}_k$ for all $k \in \left[m\right]$. This condition is satisfied as
    \begin{align*}
        ...
        &= (\log(1/2))^2 < \infty.
    \end{align*}
    \item[A3]: Fix $(x, y)\in \mathbb{X}$, $k \in \left[m\right]$ and $\beta, \beta'\in \mathbb{T}_k$. By the multivariate Mean Value Theorem, we have
    \[|\ell_k(x, y; \beta) - \ell_k(x, y; \beta')| \leq \sup_{\beta^*} \left\lVert \left. \frac{\partial \ell_k}{\partial \beta}\right|_{\beta = \beta^*} \right \rVert_2 \lVert \beta - \beta'\rVert_2, \]
...
\begin{proof}
    ...
    \item[A1]: $\ell_k(x, y; \cdot): \mathbb{T}_k \to \mathbb{R}$ is continuous for each $(x, y) \in \mathbb{X}$ as it is differentiable with respect to $\beta$. Fix $\beta \in \mathbb{T}_k$, $\ell_k(\cdot; \beta): \mathbb{X} \to \mathbb{R}$ is given by
    \begin{align*}
        \ell_k(x, y; \beta) &= \Tilde{f}(x, y; \beta)\Tilde{g}(x, y; \beta)  + \Tilde{h}_1(x, y; \beta) + \Tilde{h}_2(x, y; \beta)\\
        &= f(y; \beta)g(x; \beta)  + h_1(x; \beta) + h_2(y; \beta),
    \end{align*}
    where $\Tilde{f}(x, y; \beta) = f(y; \beta) = y$,
    \begin{align*}
        \Tilde{g}(x, y; \beta) = g(x; \beta) &= -\beta^\top x,\\
        \Tilde{h}_1(x, y; \beta) = h_1(x; \beta) &= \exp(\beta^\top x),\\
        \Tilde{h}_2(x, y; \beta) = h_2(y; \beta) &= \log(y!).
    \end{align*}
    Both $g$ and $h_1$ are continuous functions, so they are measurable with respect to the Borel $\sigma$-algebra on $\mathbb{R}^d$. Both $f$ and $h_2$ are measurable with respect to the discrete $\sigma$-algebra on $\mathbb{Z}_{\geq 0}$. So $\Tilde{f}$, $\Tilde{g}$, $\Tilde{h}_1$ and $\Tilde{h}_2$ are measurable on the product space $\mathbb{X}$. It follows that $\ell_k(\cdot; \beta)$ is measurable on $\mathbb{X}$.
    \item[A2]: $\mathbb{T}_k$ is compact. Let $\tau_k = 0 \in \mathbb{R}^d$, then $\tau_k \in \mathbb{T}_k$ for all $k \in \left[m\right]$. This condition is satisfied if $Y$ has a finite fourth moment. For any integer $y\in\mathbb{Z}_{\geq 0}$,
    \[
    ...
    \end{align*}
    \item[A3]: Fix $(x, y)\in \mathbb{X}$, $k \in \left[m\right]$ and $\beta, \beta'\in \mathbb{T}_k$. By the multivariate Mean Value Theorem,
    \[|\ell_k(x, y; \beta) - \ell_k(x, y; \beta')| \leq \sup_{\beta^*} \left\lVert \left. \frac{\partial \ell_k}{\partial \beta}\right|_{\beta = \beta^*} \right \rVert_2 \lVert \beta - \beta'\rVert_2,\]
...
\begin{proof}
    ...
    where $\Tilde{f}(x, y; \beta) = f(y; \beta) = y$,
    \begin{align*}
        \Tilde{g}(x, y; \beta) = g(x; \beta) &= \nu\exp(-\beta^\top x),\\
        \Tilde{h}_1(x, y; \beta) = h_1(x; \beta) &= \nu\beta^\top x + \log(\Gamma(\nu)) - \nu\log(\nu),\\
        \Tilde{h}_2(x, y; \beta) = h_2(y; \beta) &= - (\nu-1)\log(y).
    \end{align*}
    Both $g$ and $h_1$ are continuous functions, so they are measurable with respect to the Borel $\sigma$-algebra on $\mathbb{R}^d$. Both $f$ and $h_2$ are measurable with respect to the Borel $\sigma$-algebra on $\mathbb{R}_+$. So $\Tilde{f}$, $\Tilde{g}$, $\Tilde{h}_1$ and $\Tilde{h}_2$ are measurable on the product space $\mathbb{X}$. It follows that $\ell_k(\cdot; \beta)$ is measurable on $\mathbb{X}$.
    \item[A2]: $\mathbb{T}_k$ is compact. Let $\tau_k = 0 \in \mathbb{R}^d$, then $\tau_k \in \mathbb{T}_k$ for all $k \in \left[m\right]$. This condition is satisfied if $Y$ has a finite fourth moment and $\mathbb{E}\left\{\left(\log Y\right)^2\right\} < \infty$. Let $c_\nu = \log(\Gamma(\nu)) - \nu\log(\nu)$. Then
    \begin{align*}
...
\begin{proof}
    By Theorem 2 of \cite{nguyen_panic:_2023}, it suffices to show conditions C1--C5. We write the verification in the no-intercept notation for simplicity. If an intercept is present, define the augmented covariate $\tilde{X}=(1,X^\top)^\top$ and parameter $\tilde{\beta}=(\beta_0,\beta^\top)^\top$. Then $r(\tilde{\beta})=\mathbb{E}\left(\left(Y-\tilde{\beta}^\top \tilde{X}\right)^2\right)$ has Hessian $2\tilde{\Sigma}$ with
    \[
    \tilde{\Sigma}=\mathbb{E}(\tilde{X}\tilde{X}^\top)=\begin{bmatrix}1 & \mu^\top\\ \mu & \Sigma+\mu\mu^\top\end{bmatrix}.
    \]
    and for any $(a,b)\neq 0$ we have
\[
\begin{bmatrix}a\\ b\end{bmatrix}^\top \tilde{\Sigma}\begin{bmatrix}a\\ b\end{bmatrix}=(a+b^\top \mu)^2+b^\top \Sigma b>0
\]
whenever $\Sigma$ is positive definite. The compact interval constraining the intercept is polyhedral, and taking its Cartesian product with any of the slope constraint sets preserves the second-order regularity verified below by \citet[Proposition 3.89]{bonnans_perturbation_2000}. Hence the strict-convexity, regularity, and remaining arguments apply after this augmentation (the intercept coordinate is common to all candidate models and contributes the same amount to any absolute complexity score), so we drop tildes and omit the intercept for readability. We start with $\mathrm{C1}$. Using the functional form of $\ell_k$ from Table \ref{Table: Regression}, we have
    \begin{align*}
        r_k(\beta) &= \mathbb{E}\left(\ell_k(X, Y; \beta)\right)\\
        &= \mathbb{E}\left(\left(Y - \beta^\top X\right)^2\right)\\
        &= \mathbb{E}\left(Y^2\right) - 2\beta^\top \mathbb{E}\left(X Y\right) + \mathrm{tr}(\beta\beta^\top\Sigma) + \mu^\top \beta\beta^\top\mu,
    \end{align*}
    where $\mu$ and $\Sigma$ are the mean vector and the covariance matrix of $X$, respectively. Differentiating $r_k$ twice, we get
    \begin{align*}
        \frac{\partial^2 r_k}{\partial \beta\partial\beta^\top}(\beta) &= 2 \Sigma + 2\mu\mu^\top.
    \end{align*}
    By our assumption that $\Sigma$ is a positive definite matrix, we see that the Hessian of $r_k$ is also positive definite. Hence, $r_k$ is strictly convex. As $r_k$ is strictly convex, it has, on a convex set, at most one minimiser. Since $\mathbb{T}_k$ is compact and $r_k$ is continuous, such a minimiser exists. To show Lipschitz continuity, first, we note that our assumptions on $X$ and $Y$ satisfy the conditions in Proposition \ref{Prop: Sufficient conditions}. In particular, for $\beta, \beta' \in \mathbb{T}_k$, there exists a measurable function $\mathcal{G}_k$ with $\int \mathcal{G}_k(x,y)\,\Pi(dx,dy) < \infty$ such that for all $(x,y)\in\mathbb{R}^d\times\mathbb{R}$,
    \[
        \left| \ell_k(x, y; \beta) - \ell_k(x, y; \beta')\right| \leq \mathcal{G}_k(x, y) \lVert \beta - \beta'\rVert_1.
    \]
    Then, for $\beta, \beta' \in \mathbb{T}_k$,
    \begin{align*}
        \left|r_k(\beta) - r_k(\beta')\right| &\leq  \int_{\mathbb{R}^d \times \mathbb{R}} \left| \ell_k(x, y; \beta) - \ell_k(x, y; \beta')\right| \, \Pi(dx, dy)\\
        &\leq \int_{\mathbb{R}^d \times \mathbb{R}} \mathcal{G}_k(x, y) \lVert \beta - \beta'\rVert_1 \, \Pi(dx, dy)\\
        &= c\lVert \beta - \beta'\rVert_1,
    \end{align*}
    for some $c < \infty$. This shows $\mathrm{C1}$. Next, we note that, in our current setting, $\mathrm{C2}$ holds since $\ell_k(x,y;\beta)$ is differentiable in $\beta$ and Lipschitz continuous on $\mathbb{T}_k$. To show $\mathrm{C5}$, suppose $r_k(\beta_k^*) = r_{k^*}(\beta_{k^*}^*)$. First assume $\mathbb{T}_{k^*} \subset \mathbb{T}_{k}$. Then,
    \[
        \min_{\beta\in\mathbb{T}_k} r_k(\beta) \leq \min_{\beta\in\mathbb{T}_{k^*}} r_{k^*}(\beta),
    \]
    and $\beta_{k^*}^*$ is feasible for the larger set $\mathbb{T}_k$ and attains its minimum value. Strict convexity makes the minimiser over $\mathbb{T}_k$ unique, so $\beta_k^*=\beta_{k^*}^*$. The same argument, with the roles reversed, applies when $\mathbb{T}_{k}\subset\mathbb{T}_{k^*}$. Therefore $R_{k,n}(\beta_k^*) - R_{k^*,n}(\beta_{k^*}^*) = 0$ for all $n$, and $\mathrm{C5}$ is satisfied. In the case of Lasso regression, condition $\mathrm{C3}$ follows from the fact that $\mathbb{T}_k$ is polyhedral, for all $C_k\ge0$. For Ridge regression, the defining inequality is twice continuously differentiable and the Mangasarian--Fromovitz constraint qualification holds when $C_k>0$; when $C_k=0$, the slope set is the singleton $\{0\}$ and second-order regularity is immediate. Finally, in the case of Elastic-net regression, fix $\alpha\in(0,1)$ and note the dual representation
    \[
    \lVert\beta\rVert_1 = \max_{s\in\{-1,1\}^d} s^\top\beta.
    \]
    Hence the Elastic-net constraint is equivalent to the finite system
    \[
    (1-\alpha)\lVert\beta\rVert_2^2 + \alpha s^\top\beta \le C_k,\qquad s\in\{-1,1\}^d,
    \]
    so $\mathbb{T}_k$ can be written as the intersection $\mathbb{T}_k=\bigcap_{s\in\{-1,1\}^d}\mathbb{T}_{k,s}$ where
    \[
    \mathbb{T}_{k,s}=\left\{\beta\in\mathbb{R}^d:(1-\alpha)\lVert\beta\rVert_2^2+\alpha s^\top\beta\le C_k\right\}.
    \]
    This is a finite system of twice continuously differentiable convex inequalities. If $C_k>0$, the point $\beta=0$ is strictly feasible. Write $g_s(\beta)=(1-\alpha)\|\beta\|_2^2+\alpha s^\top\beta-C_k$. For any boundary point $\beta$ and every active inequality $g_s(\beta)=0$, convexity gives $\nabla g_s(\beta)^\top(0-\beta)<0$ because $g_s(0)<0$. Hence the Mangasarian--Fromovitz constraint qualification holds for the full finite system, and second-order regularity follows from \citet[Proposition 3.88]{bonnans_perturbation_2000}. If $C_k=0$, then $\mathbb{T}_k=\{0\}$ and second-order regularity is immediate. Condition $\mathrm{C4}$ is satisfied if the Hessian of $r_k$ is positive definite, see \citet[Appendix, discussion of C3 and C4]{nguyen_panic:_2023}. Hence, $\mathrm{C4}$ is satisfied.
\end{proof}
...
\]
is upper hemicontinuous. Since it is single-valued, it is continuous as a function \citep[Lemma 17.6]{aliprantis_infinite_2006}. Thus $\lambda\mapsto \beta_\lambda$ is continuous. Finally, $\mathcal{M}(\lambda)=g(\beta_\lambda)$ is continuous as the composition of the continuous maps $\lambda\mapsto\beta_\lambda$ and $g$.
\end{proof}

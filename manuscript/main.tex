...
Every setting has dimension $d=20$, with non-zero slope parameters indexed by $A^*=\{1,3,\ldots,19\}$. Define $\widetilde\beta_j=(-1)^{(j-1)/2}$ for $j\in A^*$ and $\widetilde\beta_j=0$ otherwise, and set
\[
...
\]
Thus CV chooses the smallest-radius minimiser in an exact tie. Its selected radius is refitted on the full sample before evaluation on the independent test sample, which does not enter the selection rule.
...
\right].
\]
We use the convention $0\log0=0$. No support or test-sample quantity enters the PanIC-CF calibration. For any reported Monte Carlo mean, the MCSE is the sample standard deviation across independent replications divided by the square root of the number of replications; paired contrasts are formed within replication before this calculation.

The primary comparison tests a joint claim against CV. For setting $s\in\{1,\ldots,7\}$ and paired replication $r$, let $W^{\rm P}_{s,r}$ and $W^{\rm C}_{s,r}$ be the total support errors $\mathrm{FP}+\mathrm{FN}$ for PanIC-CF and CV, and let $D^{\rm P}_{s,r}$ and $D^{\rm C}_{s,r}$ be their independent-test deviances. The two equal-weight seven-setting estimands are
\[
\Delta_{\rm S}
=\frac17\sum_{s=1}^7
...
\right).
\]
The analysis requires every CV test deviance to be finite and strictly positive. All 7,000 denominators satisfied this requirement, with minimum $0.90463$ and maximum $1.31098$; all realised relative contrasts were finite.
...
PanIC-CF had lower mean total support error than CV in each of the seven settings. For the equal-weight endpoint, $\widehat\Delta_{\rm S}=-0.42186$ and its one-sided $95\%$ upper Monte Carlo bound was $-0.39190$, so the support criterion passed. The relative test-deviance endpoint also passed the $0.001$ noninferiority margin, as summarised in Table \ref{Table: confirmatory decision}. The setting-specific differences are descriptive and are not simultaneous confidence statements.
...
The rows labelled BIC-like had mean total support errors from $2.33$ to $3.48$ and exact-recovery proportions from $0.064$ to $0.147$, together with larger mean test deviance than CV. In the logistic $n=500$ setting, the non-Gaussian analogue based on non-zero parameter counts had a mean of $1.61$ false negatives. These comparisons reflect different support--prediction trade-offs and do not establish a universal ranking: matching the unregularised population radius, recovering a sparse support, and minimising predictive loss are distinct objectives.
\end{bluechange}
...
Across the 7,000 replications from Section \ref{Sect: Discussion}, no fitting or calibration failure, solver warning, or multiplier default was recorded, and no raw cross-signed target required nontrivial projection. PanIC-CF selected neither a multiplier nor a radius endpoint. The logistic BIC-like comparator selected $C=0$ in 97 of the 1,000 replications at $n=500$. Calibration diagnostics include the pilot radii and selected multipliers. The remaining numerical checks are supplied with the code.
...
Each timed run used one worker after two untimed warm-up runs; data generation and independent-test evaluation were excluded. PanIC-CF had higher mean elapsed time than CV because it fits ten half-sample paths, the associated ordinary training and validation GLMs, and one full-sample path, whereas CV fits five fold-specific paths and one full-sample path. The PanIC-CF and CV means (MCSEs), respectively, were $0.0498$ ($0.0005$) and $0.0277$ ($0.0003$) seconds for Gaussian regression, $0.1715$ ($0.0036$) and $0.1030$ ($0.0010$) seconds for logistic regression, and $0.1590$ ($0.0013$) and $0.0905$ ($0.0019$) seconds for Poisson regression. The corresponding one-path means were $0.0049$ ($0.0001$) seconds for the Gaussian BIC-like criterion and $0.0189$ ($0.0002$) and $0.0164$ ($0.0001$) seconds for the logistic and Poisson analogues. These timings compare the stated implementations on one machine, not general computational complexity; the computing environment is recorded with the code referenced in Appendix \ref{App: reproducibility}.

Support-informed multiplier choices are reported only as supplementary oracle diagnostics and do not enter the primary comparisons or conclusions.
\end{bluechange}

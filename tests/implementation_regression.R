...
} else normalizePath("tests/implementation_regression.R")
test_dir <- dirname(script_path)
production_dir <- normalizePath(file.path(test_dir, ".."))
results_dir <- file.path(production_dir, "test_results")
...
write.csv(validation, file.path(results_dir, "implementation_regression.csv"),
          row.names = FALSE)

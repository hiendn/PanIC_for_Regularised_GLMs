...
audits diagnostic counts, checks the grid study, and verifies the committed
manuscript tables. Both scripts stop if a critical check fails.
...
and SHA-256 hashes from the retained production run are recorded in
`results/production_file_manifest.csv`; the compact summaries, configuration,
seed ledger, tables, and verification records are committed.
...
| `Simulation_Results_Analysis.R` | Monte Carlo summaries, joint decision, and LaTeX tables |
| `Results_Verification.R` | Independent checks of full replication-level main results |
...
| --- | --- |
| Confirmatory decision | `results/confirmatory_decision.csv`, `results/scenario_primary_estimands.csv`, `results/table_confirmatory_decision.tex` |
